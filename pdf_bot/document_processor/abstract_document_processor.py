from typing import ClassVar

from pdf_bot.document import DocumentService, DocumentServiceError
from pdf_bot.file_processor import AbstractFileProcessor
from pdf_bot.file_processor.errors import DuplicateClassError
from pdf_bot.language import LanguageService
from pdf_bot.models import TaskData
from pdf_bot.telegram_internal import TelegramService


class AbstractDocumentProcessor(AbstractFileProcessor):
    _DOCUMENT_PROCESSORS: ClassVar[dict[str, "AbstractDocumentProcessor"]] = {}

    def __init__(
        self,
        document_service: DocumentService,
        telegram_service: TelegramService,
        language_service: LanguageService,
        bypass_init_check: bool = False,
    ) -> None:
        self.document_service = document_service
        cls_name = self.__class__.__name__

        if not bypass_init_check and cls_name in self._DOCUMENT_PROCESSORS:
            raise DuplicateClassError(cls_name)
        self._DOCUMENT_PROCESSORS[cls_name] = self

        super().__init__(telegram_service, language_service, bypass_init_check)

    @classmethod
    def get_task_data_list(cls) -> list[TaskData]:
        return [x.task_data for x in cls._DOCUMENT_PROCESSORS.values()]

    @property
    def generic_error_types(self) -> set[type[Exception]]:
        return {DocumentServiceError}
