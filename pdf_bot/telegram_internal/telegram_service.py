...
class TelegramService:
    ...
    @asynccontextmanager
    async def download_file_to_dir(self, file_id: str) -> AsyncGenerator[Path, None]:
        """Download a file preserving its original name inside a temp directory."""
        file = await self.bot.get_file(file_id)
        file_name = Path(file.file_path).name if file.file_path else "document"
        with self.io_service.create_temp_directory("doc_dl") as temp_dir:
            out_path = temp_dir / file_name
            await file.download_to_drive(custom_path=out_path)
            yield out_path
